...
var dindexOfNotEqual = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( dindexOfNotEqual instanceof Error )
};
...
tape( 'the function has an arity of 5', opts, function test( t ) {
	t.strictEqual( dindexOfNotEqual.length, 5, 'has expected arity' );
	t.end();
});

tape( 'the function returns the index of the first element in a strided array which is not equal to a provided search element', opts, function test( t ) {
	var actual;
	var x;
...
tape( 'the function returns `-1` if every indexed element in the array is equal to the search element', opts, function test( t ) {
	var actual;
	var x;
...
tape( 'the function returns `-1` if a provided `N` parameter is less than or equal to zero', opts, function test( t ) {
	...
	actual = dindexOfNotEqual( 0, 2.0, x, 1, 0 );
	t.strictEqual( actual, -1, 'returns expected value' );

	actual = dindexOfNotEqual( -1, 2.0, x, 1, 0 );
	t.strictEqual( actual, -1, 'returns expected value' );

	t.end();
});

tape( 'the function returns the index of the first element if a provided `searchElement` is `NaN`', opts, function test( t ) {
	var actual;
	var x;

	x = new Float64Array( [ NaN, 1.0 ] );

	actual = dindexOfNotEqual( 2, NaN, x, 1, 0 );
	t.strictEqual( actual, 0, 'returns expected value' );
...
tape( 'the function supports an `x` stride', opts, function test( t ) {
	...
	actual = dindexOfNotEqual( 3, 0.0, x, 2, 0 );
	t.strictEqual( actual, 2, 'returns expected value' );
